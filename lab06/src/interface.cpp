...
void press_enter() {
	std::cout << DEFAULT FLASHING "<press Enter to continue>" << std::endl;
	if (getchar() != '\n')
		clear_input();
	std::cout << CANCEL_FLUSHING DEFAULT " " CLS;
}
...
void remove_key(HashTable<int, MyHasher<int> >& hash_table,
	...
	a = hash_table.remove(key);
	auto _b = std::chrono::steady_clock::now();
	*avl_tree = remove(*avl_tree, key, b);
	auto _c = std::chrono::steady_clock::now();
	*binary_tree = remove(*binary_tree, key, c);
