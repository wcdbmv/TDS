...
static std::vector<size_t> freed;
...
#define PLAN_A2 (rand_double(options.a2.from, options.a2.to, EXP))
	...
	while (processed1 < options.num) {
		...
		if (!oa.processing) {
			if (!q1->is_empty()) {
				freed.push_back(q1->vtop());
				load(oa, q1->pop(), timepass + PLAN_A1);
			}
			else if (!q2->is_empty()) {
				freed.push_back(q2->vtop());
		...
		if (!(processed1 % options.period)) {
			if (!action) {
				...
				if (options.type == 2)
					fprintf(stderr, "\tAdresses of top: Q1 = 0x%lX;   Q2 = 0x%lX\n", q1->vtop(), q2->vtop());
				...
				if (options.type == 2)
					fprintf(stdout, "\tDefinitely free'd but not alloc again: %lu\n", freed.size());
